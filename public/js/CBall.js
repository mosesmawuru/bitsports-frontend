function CBall(iType, oParentContainer, oTexture) {
    ...
    if (DEBUG_SHOW_2D_SPRITE) {
      var oData = {
        ...
        frames: {
          ...
          regX: BALL_DIAMETER / 2,
          regY: BALL_DIAMETER / 2,
        },
        animations: {
          ball_0: 0,
          ball_1: 1,
  ...
  this.enableEvents = function () {
    ...
    _oBallHitArea.graphics
      .beginFill("#aaa")
      .drawRect(
        -CUE_BALL_HIT_AREA_DIMENSION * 0.5,
        -CUE_BALL_HIT_AREA_DIMENSION * 0.5,
  ...
  this.setDragging = function (bDrag) {
    ...
    // console.log("setDragging "+bDrag);
    // console.log("ball pos "+_oBall3D.position.x+" "+_oBall3D.position.y+" "+_oBall3D.position.z);
  };
  ...
  this.setMask = function (iXMask, iYMask) {
    ...
    _oMask.graphics
      ...
      .drawCircle(iXMask, iYMask, POOL_HOLE_RADIUS);
  ...
  this.addPos = function (vPos) {
    _vPos.add(vPos);
    // console.log("addPos "+_iNumber+" "+vPos.getX()+" "+vPos.getY());

    // var b = this.getNumber();
    // var x = vPos.getX();
    // var y = vPos.getY();

    // socket.emit('cueballvel', {b, x, y} , (error) => {
    //         if (error) {
    //             alert(error)
    //             location.href = '/'
    //         }
  ...
  this.fadeAnimInHole = function (iTime) {
    ...
    createjs.Tween.get(oMaterial, { override: true }).to(
      { opacity: 0 },
      iTime,
    ...
    createjs.Tween.get(vScale)
      .to({ x: 0.1, y: 0.1, z: 0.1 }, iTime, createjs.Ease.sineOut)
      .call(
        function () {
  ...
  this._updateShadow = function () {
    ...
    var vDirCenter = new CVector2(
      TABLE_CENTER_COORDINATE.getX(),
      TABLE_CENTER_COORDINATE.getY()
    );
    ...
    var fFade = linearFunction(vDirCenter.length(), 770, 0, 0, 1);
  ...
  this.addEventListener = function (szEvt, cbCompleted, cbOwner, aParams) {
    ...
    _aListeners[szEvt] = _oBallHitArea.on(
      szEvt,
      this.triggerEvent,
      this,
      false,
